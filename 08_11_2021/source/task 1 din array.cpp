...
void createUniSpiral(int** A, int row, int column, int start1) {
    int max = 0;
    if (row >= column) {
        max = row;
    }
...
{/*
    ...
    case 6 :std::cout << "arrage-UNIspiral" << std::endl;
        createUniSpiral(A, row, column, start1);
        printArray(A, row, column);
        std::cout << std::endl;
        break;
